...
# Rigorous THEIA E1 TBB-RR redundancy comparison.
# Strategy:
# 1. Run a single-pass strict split search on chronological THEIA events.
...
#
# Default lab usage:
#   bash scripts/run_theia_tbb_rr_verdict.sh
#
# Useful overrides:
#   WINDOW_EVENTS=80000 bash scripts/run_theia_tbb_rr_verdict.sh
#   SEEDS="42 43 44" bash scripts/run_theia_tbb_rr_verdict.sh
#   TBB_RR_TARGET_COMPRESSION=0.90 bash scripts/run_theia_tbb_rr_verdict.sh
#   RUN_OFF=0 bash scripts/run_theia_tbb_rr_verdict.sh
...
export WINDOW_EVENTS="${WINDOW_EVENTS:-200000}"
export GRAPH_LIMIT_TRAIN="${GRAPH_LIMIT_TRAIN:-36}"
export GRAPH_LIMIT_VAL="${GRAPH_LIMIT_VAL:-12}"
export GRAPH_LIMIT_TEST="${GRAPH_LIMIT_TEST:-12}"
...
export RAW_DIR="${RAW_DIR:-data/raw/darpa_tc/theia/e3/cdm}"
export LABEL_DIR="${LABEL_DIR:-data/raw/darpa_tc/theia/e3/labels}"
export RAW_GLOB="${RAW_GLOB:-ta1-theia-e3-official*.json*}"
...
export STRICT_MAX_EVENTS_CAP="${STRICT_MAX_EVENTS_CAP:-12000000}"
...
# numeric event weight side channel, including TBB-RR.
...
BASE_OUT="${BASE_OUT:-runs/theia_tbb_rr_verdict_${TS}_autostop_win${WINDOW_EVENTS}}"
...
collect_tbb_rr_bundle() {
  if [[ ! -d "$BASE_OUT" ]]; then
    return 0
  fi
  mkdir -p "$BUNDLE_DIR"
  if [[ -f scripts/collect_theia_tbb_rr_analysis_bundle.sh ]]; then
    ...
    bash scripts/collect_theia_tbb_rr_analysis_bundle.sh "$BASE_OUT" "$BUNDLE_DIR" \
      > "$BUNDLE_DIR/collect_tbb_rr_analysis_bundle.log" 2>&1 || {
        echo "[WARN] failed to collect TBB-RR analysis bundle; see $BUNDLE_DIR/collect_tbb_rr_analysis_bundle.log" >&2
      }
  else
    echo "[WARN] scripts/collect_theia_tbb_rr_analysis_bundle.sh not found; no single-folder bundle created" >&2
  fi
}
trap collect_tbb_rr_bundle EXIT

cat > "$BASE_OUT/STRICT_EXPERIMENT_PLAN.md" <<EOF
# Rigorous THEIA E1 TBB-RR redundancy experiment (autostop)

- target_model: E1_eha_only
- comparison: off vs prefix_tree vs target_boundary
...
- require split-wise process-node positive/negative mixture, which matches the
  actual supervision target of E1_eha_only on THEIA;
...
cfg = {
    ...
    "seed": 42,
    "dataset_name": "theia_e3_tbb_rr_strict_precheck_autostop",
...
echo "[precheck] searching earliest rigorous THEIA prefix..."
...
echo "[run] launching THEIA TBB-RR verdict on the rigor-qualified prefix..."
MATRIX_FILE="$EXPERIMENT_OUT/run_matrix.tsv"
printf "label\tredundancy_mode\ttarget\tbeta\trole\tcache_root\n" > "$MATRIX_FILE"
...
run_mode() {
  ...
  mkdir -p "$out"
  local cache_root="$out/cache/${short}_events${SELECTED_MAX_EVENTS}_win${WINDOW_EVENTS}_mode${mode}_tbb${TBB_RR_TARGET_COMPRESSION}_gs${GRAPH_SIMPLIFY_MODE}"
  printf "%s\t%s\t%s\t%s\t%s\t%s\n" "$short" "$mode" "$target_param" "$beta" "$role" "$cache_root" >> "$MATRIX_FILE"
  echo "========== label=$short redundancy_mode=$mode target=${target_param:-na} beta=${beta:-na} cache_root=$cache_root =========="
  (
    export PARENT_OUT="$out"
    export REDUNDANCY_MODE="$mode"
    export DATASET_NAME="theia_e3_tbb_rr_${short}"
    # Critical: the generic CADETS runner uses CADETS_CACHE_ROOT for automatic
    # graph-cache reuse.  For THEIA redundancy comparisons this must be
    # different for every redundancy mode; otherwise prefix_tree/TBB can
    # accidentally reuse an off-mode cache and make the comparison invalid.
    export CADETS_CACHE_ROOT="$cache_root"
    unset THEIA_CACHE_ROOT
  ...
  echo "[cache-check] validating effective cache for label=$short mode=$mode"
  python - "$cache_root" "$mode" "$TBB_RR_TARGET_COMPRESSION" "$out/CACHE_MODE_VALIDATION.json" <<'PY'
import json
import sys
from pathlib import Path
...
expected_tbb_target = float(sys.argv[3])
out_json = Path(sys.argv[4])
meta_path = cache_root / "analysis" / "preprocess" / "metadata.json"
if not meta_path.exists():
    raise SystemExit(f"[cache-check][FAIL] metadata missing: {meta_path}")
meta = json.loads(meta_path.read_text(encoding="utf-8"))
cfg = meta.get("config") or {}
stats = meta.get("stats") or []
config_mode = str(cfg.get("redundancy_mode", "")).lower()
stat_modes = sorted({str((s or {}).get("redundancy_mode", "")).lower() for s in stats if isinstance(s, dict)})
before = sum(int((s or {}).get("node_events_before_reduction", 0) or 0) for s in stats if isinstance(s, dict))
after = sum(int((s or {}).get("node_events_after_reduction", 0) or 0) for s in stats if isinstance(s, dict))
ratio = (before - after) / max(before, 1)
errors = []
if config_mode != expected_mode.lower():
    errors.append(f"config.redundancy_mode={config_mode!r} expected {expected_mode!r}")
if stat_modes and any(m != expected_mode.lower() for m in stat_modes):
    errors.append(f"stats.redundancy_mode={stat_modes!r} expected only {expected_mode!r}")
if expected_mode.lower() == "off":
    if abs(ratio) > 1e-12:
        errors.append(f"off mode should not reduce events, got ratio={ratio:.6f}")
else:
    if before <= 0:
        errors.append("reduced mode has no node_events_before_reduction statistics")
    if ratio <= 0.001:
        errors.append(f"reduced mode produced near-zero event reduction ratio={ratio:.6f}")
if expected_mode.lower() == "target_boundary":
    actual_target = float(cfg.get("tbb_rr_target_compression", -1) or -1)
    if abs(actual_target - expected_tbb_target) > 1e-9:
        errors.append(f"tbb_rr_target_compression={actual_target} expected {expected_tbb_target}")
result = {
    "metadata": str(meta_path),
    "expected_mode": expected_mode,
    "config_mode": config_mode,
    "stat_modes": stat_modes,
    "node_events_before_reduction": before,
    "node_events_after_reduction": after,
    "node_event_reduction_ratio": ratio,
    "expected_tbb_rr_target_compression": expected_tbb_target if expected_mode.lower() == "target_boundary" else None,
    "config_tbb_rr_target_compression": cfg.get("tbb_rr_target_compression"),
...
run_mode "target_boundary" "target_boundary" "$TBB_RR_TARGET_COMPRESSION" "$MW_PRR_ATTENTION_BETA" "tbb_rr_candidate"
...
fields = [
    "label", "redundancy_mode", "target", "beta", "role", "seed", "experiment",
    "f1", "precision", "recall", "mcc", "average_precision", "roc_auc",
...
for row in rows:
    for key in row:
        if key not in fields:
            fields.append(key)
summary_path = root / "summary_theia_tbb_rr.csv"
...
for spec in matrix:
    ...
    if not bucket:
        continue
    item = {k: spec.get(k, "") for k in ["label", "redundancy_mode", "target", "beta", "role"]}
    item.update({
        "n_seeds": len(bucket),
...
agg_fields = [
    "label", "redundancy_mode", "target", "beta", "role", "n_seeds", "num_graphs",
...
agg_path = root / "summary_theia_tbb_rr_agg.csv"
...
for spec in matrix:
    ...
    for row in by_label_rows.get(label, []):
        ...
        if not base:
            continue
        item = {"label": label, "mode": spec.get("redundancy_mode", ""), "target": spec.get("target", ""), "seed": row["seed"]}
...
pair_fields = ["label", "mode", "target", "seed", "candidate_f1", "prefix_f1"] + [f"delta_{k}" for k in primary_metrics + ["train_seconds"]]
...
for label, bucket in by_pair_label.items():
    item = {"label": label, "mode": bucket[0].get("mode", ""), "target": bucket[0].get("target", ""), "n_paired_seeds": len(bucket)}
...
paired_agg_fields = ["label", "mode", "target", "n_paired_seeds"]
for key in primary_metrics:
...
report = root / "THEIA_TBB_RR_VERDICT_REPORT.md"
with report.open("w", encoding="utf-8") as f:
    f.write("# TBB-RR redundancy verdict report\n\n")
    f.write("This report compares off, MalSnif prefix_tree, and TBB-RR target_boundary on the same strict-qualified THEIA prefix and paired seeds. TBB-RR is a one-parameter target-budget boundary reducer; it should not be claimed as an improvement unless paired deltas exceed the predefined rule.\n\n")
    f.write("## Mean +/- std across seeds\n\n")
    f.write("| label | mode | target | seeds | F1 | Precision | Recall | MCC | AP | ROC-AUC | Train(s) | reduction | weight_mean | high_weight_ratio |\n")
    f.write("|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    for r in agg_rows:
        f.write(
            f"| {r.get('label')} | {r.get('redundancy_mode')} | {r.get('target')} | {r.get('n_seeds')} | "
    ...
    f.write("| label | mode | target | paired seeds | delta F1 | F1 win/loss/tie | delta MCC | delta Recall | verdict |\n")
    f.write("|---|---|---:|---:|---:|---|---:|---:|---|\n")
    for item in paired_agg:
        f.write(
            f"| {item.get('label')} | {item.get('mode')} | {item.get('target')} | {item.get('n_paired_seeds')} | "
...
echo "[done] summary=$ABS_EXPERIMENT_OUT/summary_theia_tbb_rr.csv"
echo "[done] aggregate_summary=$ABS_EXPERIMENT_OUT/summary_theia_tbb_rr_agg.csv"
echo "[done] report=$ABS_EXPERIMENT_OUT/THEIA_TBB_RR_VERDICT_REPORT.md"
