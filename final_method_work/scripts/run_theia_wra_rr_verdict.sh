...
# Rigorous THEIA E1 WRA-RR redundancy comparison.
# Strategy:
...
#
# Default lab usage:
#   bash scripts/run_theia_wra_rr_verdict.sh
#
# Useful overrides:
#   WINDOW_EVENTS=80000 bash scripts/run_theia_wra_rr_verdict.sh
#   SEEDS="42 43 44" bash scripts/run_theia_wra_rr_verdict.sh
#   WRA_RR_WINDOW=11 bash scripts/run_theia_wra_rr_verdict.sh
#   RUN_OFF=0 bash scripts/run_theia_wra_rr_verdict.sh
...
BASE_OUT="${BASE_OUT:-runs/theia_wra_rr_verdict_${TS}_autostop_win${WINDOW_EVENTS}}"
...
collect_wra_rr_bundle() {
  ...
  mkdir -p "$BUNDLE_DIR"
  if [[ -f scripts/collect_theia_wra_rr_analysis_bundle.sh ]]; then
    ...
    bash scripts/collect_theia_wra_rr_analysis_bundle.sh "$BASE_OUT" "$BUNDLE_DIR" \
    ...
    echo "[WARN] scripts/collect_theia_wra_rr_analysis_bundle.sh not found; no single-folder bundle created" >&2
  fi
}
trap collect_wra_rr_bundle EXIT

cat > "$BASE_OUT/STRICT_EXPERIMENT_PLAN.md" <<EOF
# Rigorous THEIA E1 WRA-RR redundancy experiment (autostop)
...
cfg = {
    ...
    "seed": 42,
    "dataset_name": "theia_e3_wra_rr_strict_precheck_autostop",
...
echo "[run] launching THEIA WRA-RR verdict on the rigor-qualified prefix..."
MATRIX_FILE="$EXPERIMENT_OUT/run_matrix.tsv"
printf "label\tredundancy_mode\twindow\tbeta\trole\tcache_root\n" > "$MATRIX_FILE"
...
run_mode() {
  ...
  mkdir -p "$out"
  local cache_root="$out/cache/${short}_events${SELECTED_MAX_EVENTS}_win${WINDOW_EVENTS}_mode${mode}_wra${WRA_RR_WINDOW}_gs${GRAPH_SIMPLIFY_MODE}"
  printf "%s\t%s\t%s\t%s\t%s\t%s\n" "$short" "$mode" "$window_param" "$beta" "$role" "$cache_root" >> "$MATRIX_FILE"
  echo "========== label=$short redundancy_mode=$mode window=${window_param:-na} beta=${beta:-na} cache_root=$cache_root =========="
  (
    export PARENT_OUT="$out"
    export REDUNDANCY_MODE="$mode"
    export DATASET_NAME="theia_e3_wra_rr_${short}"
    ...
    # different for every redundancy mode; otherwise prefix_tree/WRA can
  ...
  python - "$cache_root" "$mode" "$WRA_RR_WINDOW" "$out/CACHE_MODE_VALIDATION.json" <<'PY'
import json
import sys
from pathlib import Path
...
expected_wra_window = int(sys.argv[3])
...
if expected_mode.lower() == "winnowing_anchor":
    actual_window = int(cfg.get("wra_rr_window", -1) or -1)
    if actual_window != expected_wra_window:
        errors.append(f"wra_rr_window={actual_window} expected {expected_wra_window}")
result = {
    ...
    "expected_wra_rr_window": expected_wra_window if expected_mode.lower() == "winnowing_anchor" else None,
    "config_wra_rr_window": cfg.get("wra_rr_window"),
...
for row in rows:
    for key in row:
        if key not in fields:
            fields.append(key)
summary_path = root / "summary_theia_wra_rr.csv"
...
agg_path = root / "summary_theia_wra_rr_agg.csv"
...
report = root / "THEIA_WRA_RR_VERDICT_REPORT.md"
with report.open("w", encoding="utf-8") as f:
    f.write("# WRA-RR redundancy verdict report\n\n")
    f.write("This report compares off, MalSnif prefix_tree, and WRA-RR winnowing_anchor on the same strict-qualified THEIA prefix and paired seeds. WRA-RR is a one-parameter winnowing representative-anchor reducer; it should not be claimed as an improvement unless paired deltas exceed the predefined rule.\n\n")
...
echo "[done] summary=$ABS_EXPERIMENT_OUT/summary_theia_wra_rr.csv"
echo "[done] aggregate_summary=$ABS_EXPERIMENT_OUT/summary_theia_wra_rr_agg.csv"
echo "[done] report=$ABS_EXPERIMENT_OUT/THEIA_WRA_RR_VERDICT_REPORT.md"
