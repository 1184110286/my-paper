...
# Collect lightweight, analysis-critical artifacts for the THEIA TBB-RR one-key run.
...
#
# Usage:
#   bash scripts/collect_tbb_rr_analysis_bundle.sh <run_root> [bundle_dir]
...
if [[ ${#labels[@]} -eq 0 ]]; then
  labels=(off prefix_tree target_boundary)
fi
...
for mode in "${labels[@]}"; do
  ...
  # Current outer TBB script launches each mode as a child v3 run, whose useful
...
preferred = ['off', 'prefix_tree', 'target_boundary']
...
write_csv(out / 'experiment__summary_theia_tbb_rr_fallback.csv', sorted(rows, key=lambda r:(r['mode'],r['seed'])), fields)
...
write_csv(out / 'experiment__summary_theia_tbb_rr_agg_fallback.csv', agg_rows, agg_fields)
...
report = out / 'experiment__THEIA_TBB_RR_VERDICT_REPORT_fallback.md'
with open(report, 'w', encoding='utf-8') as fh:
    fh.write('# THEIA TBB-RR fallback verdict report\n\n')
    fh.write('This report was generated by collect_tbb_rr_analysis_bundle.sh because the outer run did not contain final aggregate files.\n\n')
...
script=scripts/run_theia_tbb_rr_verdict.sh
run_root=$RUN_ROOT
...
{
  echo "# Single-folder THEIA TBB-RR analysis bundle"
...
echo "[bundle] THEIA TBB-RR single-folder analysis bundle collected in $COLLECTED_DIR"
