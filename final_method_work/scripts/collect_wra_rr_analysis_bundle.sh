...
# Collect lightweight, analysis-critical artifacts for the WRA-RR one-key run.
...
write_csv(out / 'experiment__summary_wra_rr_fallback.csv', sorted(rows, key=lambda r:(r['mode'],r['seed'])), fields)
...
write_csv(out / 'experiment__summary_wra_rr_agg_fallback.csv', agg_rows, agg_fields)
...
report = out / 'experiment__WRA_RR_VERDICT_REPORT_fallback.md'
with open(report, 'w', encoding='utf-8') as fh:
    fh.write('# WRA-RR fallback verdict report\n\n')
...
script=scripts/run_cadets_wra_rr_verdict.sh
run_root=$RUN_ROOT
...
{
  echo "# Single-folder WRA-RR analysis bundle"
...
echo "[bundle] WRA-RR single-folder analysis bundle collected in $COLLECTED_DIR"
