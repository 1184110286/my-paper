...
# Rigorous CADETS E1 TBB-RR redundancy comparison.
# Strategy:
...
#
# Default lab usage:
#   bash scripts/run_cadets_tbb_rr_verdict.sh
#
# Useful overrides:
#   WINDOW_EVENTS=80000 bash scripts/run_cadets_tbb_rr_verdict.sh
#   SEEDS="42 43 44" bash scripts/run_cadets_tbb_rr_verdict.sh
#   TBB_RR_TARGET_COMPRESSION=0.90 bash scripts/run_cadets_tbb_rr_verdict.sh
#   RUN_OFF=0 bash scripts/run_cadets_tbb_rr_verdict.sh
...
BASE_OUT="${BASE_OUT:-runs/cadets_tbb_rr_verdict_${TS}_autostop_win${WINDOW_EVENTS}}"
...
collect_tbb_rr_bundle() {
  ...
  mkdir -p "$BUNDLE_DIR"
  if [[ -f scripts/collect_tbb_rr_analysis_bundle.sh ]]; then
    ...
    bash scripts/collect_tbb_rr_analysis_bundle.sh "$BASE_OUT" "$BUNDLE_DIR" \
    ...
    echo "[WARN] scripts/collect_tbb_rr_analysis_bundle.sh not found; no single-folder bundle created" >&2
  fi
}
trap collect_tbb_rr_bundle EXIT

cat > "$BASE_OUT/STRICT_EXPERIMENT_PLAN.md" <<EOF
# Rigorous CADETS E1 TBB-RR redundancy experiment (autostop)
...
cfg = {
    ...
    "seed": 42,
    "dataset_name": "cadets_e3_tbb_rr_strict_precheck_autostop",
...
echo "[run] launching TBB-RR verdict on the rigor-qualified prefix..."
MATRIX_FILE="$EXPERIMENT_OUT/run_matrix.tsv"
printf "label\tredundancy_mode\ttarget\tbeta\trole\n" > "$MATRIX_FILE"
...
run_mode() {
  ...
  mkdir -p "$out"
  printf "%s\t%s\t%s\t%s\t%s\n" "$short" "$mode" "$target_param" "$beta" "$role" >> "$MATRIX_FILE"
  echo "========== label=$short redundancy_mode=$mode target=${target_param:-na} beta=${beta:-na} =========="
  (
    export PARENT_OUT="$out"
    export REDUNDANCY_MODE="$mode"
    export DATASET_NAME="cadets_e3_tbb_rr_${short}"
...
for row in rows:
    for key in row:
        if key not in fields:
            fields.append(key)
summary_path = root / "summary_tbb_rr.csv"
...
agg_path = root / "summary_tbb_rr_agg.csv"
...
report = root / "TBB_RR_VERDICT_REPORT.md"
with report.open("w", encoding="utf-8") as f:
    f.write("# TBB-RR redundancy verdict report\n\n")
    f.write("This report compares off, MalSnif prefix_tree, and TBB-RR target_boundary on the same strict-qualified CADETS prefix and paired seeds. TBB-RR is a one-parameter target-budget boundary reducer; it should not be claimed as an improvement unless paired deltas exceed the predefined rule.\n\n")
...
echo "[done] summary=$ABS_EXPERIMENT_OUT/summary_tbb_rr.csv"
echo "[done] aggregate_summary=$ABS_EXPERIMENT_OUT/summary_tbb_rr_agg.csv"
echo "[done] report=$ABS_EXPERIMENT_OUT/TBB_RR_VERDICT_REPORT.md"
