...
# Collect lightweight, analysis-critical artifacts for the TBB-RR one-key run.
...
write_csv(out / 'experiment__summary_tbb_rr_fallback.csv', sorted(rows, key=lambda r:(r['mode'],r['seed'])), fields)
...
write_csv(out / 'experiment__summary_tbb_rr_agg_fallback.csv', agg_rows, agg_fields)
...
report = out / 'experiment__TBB_RR_VERDICT_REPORT_fallback.md'
with open(report, 'w', encoding='utf-8') as fh:
    fh.write('# TBB-RR fallback verdict report\n\n')
...
script=scripts/run_cadets_tbb_rr_verdict.sh
run_root=$RUN_ROOT
...
{
  echo "# Single-folder TBB-RR analysis bundle"
...
echo "[bundle] TBB-RR single-folder analysis bundle collected in $COLLECTED_DIR"
