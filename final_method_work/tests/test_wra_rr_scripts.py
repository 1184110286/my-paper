...
def test_wra_runner_is_three_way_only_and_uses_single_folder_bundle():
    script = Path("scripts/run_cadets_wra_rr_verdict.sh").read_text(encoding="utf-8")
    ...
    assert 'run_mode "btr_rr"' not in script
    assert 'run_mode "lz_skeleton"' not in script
    assert 'run_mode "weighted_prefix_tree"' not in script
    assert "collect_wra_rr_analysis_bundle.sh" in script
    assert "single_folder_analysis_bundle" in script
    assert "[send me]" in script


def test_wra_collect_bundle_flattens_child_seed_outputs(tmp_path):
    run_root = tmp_path / "runs" / "cadets_wra_rr_demo"
    ...
    (exp / "summary_wra_rr.csv").write_text("label,f1\n", encoding="utf-8")
    subprocess.run(["bash", "scripts/collect_wra_rr_analysis_bundle.sh", str(run_root)], check=True)
    collected = run_root / "analysis_bundle" / "collected"
    assert collected.exists()
    assert (collected / "experiment__summary_wra_rr.csv").exists()
